# This script uses on export dataset. For each product the working URL is visited and textual description of the product
...
def main():
    ...
    input_file_df = output_path + 'df_clean_export.p'
    output_file_pd = output_path + "Product_description_export.txt" # to store textual description for each product
    output_file_df = output_path + "df_text_extract_export.p" # to store the dataframe obtained after appending product description as pickle object
    ...
    df = df[df['product_id'].isin([11,12,13,14])]
    pid = df['product_id']
